...
class _AddPropertyState extends ConsumerState<AddProperty> {
  ...
  static const List<String> _amenitiesList = [
    'School Nearby',
    'Hospital Nearby',
    'Market Nearby',
    'Bus Stop',
    ...
    return Column(
      ...
      children: [
        _field(
          ...
          _titleCtrl,
          required: true,
          hint: 'e.g. Spacious 3-Bedroom Villa',
        ),
        ...
        Wrap(
          ...
          children: _amenitiesList.map((amenity) {
            ...
            return GestureDetector(
              onTap: () => setState(
                () => selected
                    ? _nearbyAmenities.remove(amenity)
                    : _nearbyAmenities.add(amenity),
              ),
  ...
  Widget _buildReviewStep(ThemeData theme) {
    return Column(
      ...
      children: [
        _reviewRow(
          'Title',
          _titleCtrl.text.isEmpty ? 'Not set' : _titleCtrl.text,
        ),
        _reviewRow(
          'Location',
          _locationCtrl.text.isEmpty ? 'Not set' : _locationCtrl.text,
        ),
        _reviewRow(
          'Monthly Rent',
          _rentCtrl.text.isEmpty
              ? 'Not set'
              : '${CurrencyFormatter.formatCFA(double.tryParse(_rentCtrl.text) ?? 0)} / mo',
        ),
        _reviewRow('Category', _category),
        _reviewRow('Bedrooms / Baths', '$_bedrooms bed / $_bathrooms bath'),
        _reviewRow(
          'Area',
          '${_areaCtrl.text.isEmpty ? '—' : _areaCtrl.text} m²',
        ),
  ...
  void _publishProperty() async {
    ...
    try {
      ...
      if (_floorPlanImages.isNotEmpty) {
        floorPlans = await StorageService.instance.uploadMultipleFiles(
          _floorPlanImages,
          'property-images',
        );
      }
      ...
      await repo.publishProperty(property.id);
    } catch (e) {
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            const Text(
              'Your property is now live on SpaceRentals. Tenants can start browsing and applying.',
