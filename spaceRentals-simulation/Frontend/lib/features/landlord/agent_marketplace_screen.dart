...
class AgentMarketplaceScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AgentMarketplaceScreen> createState() =>
      _AgentMarketplaceScreenState();
}

class _AgentMarketplaceScreenState
    extends ConsumerState<AgentMarketplaceScreen> {
  String _searchQuery = '';
            ...
            child: agentsAsync.when(
              ...
              data: (agents) {
                ...
                final activeAgents = agents
                    .where((a) => a.status == 'active' && !a.isWalletFrozen)
                    .toList();
                final filtered = activeAgents
                    .where(
                      (a) =>
                          a.name.toLowerCase().contains(_searchQuery) ||
                          a.location.toLowerCase().contains(_searchQuery) ||
                          a.areasServed.any(
                            (area) => area.toLowerCase().contains(_searchQuery),
                          ),
                    )
                    .toList();
                ...
                if (filtered.isEmpty) {
                  return const Center(
                    child: Text('No agents found matching your criteria.'),
                  );
                }
...
class _AgentCard extends ConsumerWidget {
  ...
  Future<void> _requestService(BuildContext context, WidgetRef ref) async {
    try {
      await ref
