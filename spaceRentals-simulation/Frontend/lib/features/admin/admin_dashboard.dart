...
class _AdminOverviewScreen extends ConsumerWidget {
      ...
      body: allUsersAsync.when(
        ...
        data: (allUsers) => kycListAsync.when(
          ...
          data: (kycList) => disputesAsync.when(
            ...
            data: (disputes) {
              final tenants = allUsers
                  .where((u) => u.role == Role.tenant)
                  .toList();
              final landlords = allUsers
                  .where((u) => u.role == Role.landlord)
                  .toList();
              final admins = allUsers
                  .where((u) => u.role == Role.admin)
                  .toList();
              final pendingKYC = kycList
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (pendingKYC.length > 3)
                            _ViewAllButton(
                              label:
                                  'View all ${pendingKYC.length} KYC submissions',
                              onTap: () =>
                                  _showKYCBottomSheet(context, ref, kycList),
                            ),
                          if (pendingKYC.isNotEmpty && pendingKYC.length <= 3)
                            _ViewAllButton(
                              label: 'View all KYC submissions',
                              onTap: () =>
                                  _showKYCBottomSheet(context, ref, kycList),
                            ),
                          const SizedBox(height: 28),
                          ...
                          if (disputes.length > 3)
                            _ViewAllButton(
                              label: 'View all ${disputes.length} disputes',
                              onTap: () => _showDisputesBottomSheet(
                                context,
                                ref,
                                disputes,
                              ),
                          ...
                          _ManagementTile(
                            ...
                            onTap: () =>
                                _showAddAdminDialog(context, ref, theme),
                          ),
                          ...
                          _ManagementTile(
                            ...
                            subtitle:
                                'Clean up test/dummy accounts from platform',
                            onTap: () => _showRemoveTestDialog(
                              context,
                              ref,
                              allUsers,
                              theme,
                          ...
                          ...admins.map(
                            (a) => _AdminCard(
                              admin: a,
                              currentAdminId: admin.session?.userId ?? '',
                              ref: ref,
                              theme: theme,
                            ),
                          ),
  ...
  void _showRemoveTestDialog(
    BuildContext context,
    WidgetRef ref,
    List<UserModel> users,
    ThemeData theme,
  ) {
    final testAccounts = users
        .where(
          (u) =>
              u.email.contains('test') ||
              u.email.contains('dummy') ||
              u.email.contains('mock') ||
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: testAccounts.isEmpty
            ...
            : Column(
                ...
                children: [
                  Text(
                    'Found ${testAccounts.length} test account(s):',
                    style: const TextStyle(fontWeight: FontWeight.bold),
                  ),
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Text(
                submission.isPremium ? 'Premium KYC' : 'Basic KYC',
                style: const TextStyle(
                  fontSize: 12,
                  fontWeight: FontWeight.w600,
                ),
          ...
          if (submission.documents.isNotEmpty) ...[
            ...
            Wrap(
              spacing: 12,
              runSpacing: 12,
              children: submission.documents.entries
                  .map(
                    (entry) => GestureDetector(
                      onTap: () =>
                          _showDocumentFullscreen(context, entry.value),
                      child: Column(
                        children: [
                          Container(
                            width: 80,
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: alert ? color.withValues(alpha: 0.4) : Colors.grey.shade100,
          ),
          boxShadow: [
            BoxShadow(
              color: Colors.black.withValues(alpha: 0.04),
