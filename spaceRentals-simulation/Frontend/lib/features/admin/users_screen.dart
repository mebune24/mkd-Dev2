...
class UsersScreen extends ConsumerWidget {
  ...
  Future<void> _toggleSuspend(
    ...
    final action = current.status == 'active' ? 'suspend' : 'activate';
    final response = await ref
        .read(apiClientProvider)
        .patch('/api/admin/users/${current.id}/$action');
    if (!response.isSuccess) {
      if (context.mounted) {
        context.showErrorToast(
          response.error?.message ?? 'User status update failed',
        );
      }
      return;
    }
    ref.invalidate(allUsersProvider);
    if (context.mounted) context.showSuccessToast('User status updated.');
  }
              ...
              trailing: user.role != Role.admin
                  ? PopupMenuButton<String>(
                      onSelected: (value) async {
                        if (value == 'toggle') {
                          await _toggleSuspend(context, ref, user);
                        }
                      },
                      itemBuilder: (context) => [
                        PopupMenuItem(
