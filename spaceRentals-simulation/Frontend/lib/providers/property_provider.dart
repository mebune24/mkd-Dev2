...
final marketplaceListingsProvider = FutureProvider<List<PropertyWithListing>>((
  ref,
) async {
...
final videoFeedProvider = FutureProvider<VideoFeedPage>((ref) async {
  return ref.watch(videoFeedRepositoryProvider).getVideoFeed();
});

/// Provides properties filtered by a specific category
final propertiesByCategoryProvider =
    FutureProvider.family<List<PropertyWithListing>, String>((
      ref,
      category,
    ) async {
      final repo = ref.watch(propertyRepositoryProvider);
...
final propertiesBySearchProvider =
    FutureProvider.family<List<PropertyWithListing>, String>((
      ref,
      query,
    ) async {
...
final propertyDetailProvider =
    FutureProvider.family<PropertyWithListing, String>((ref, id) async {
      final repo = ref.watch(propertyRepositoryProvider);
...
final landlordPropertiesProvider = FutureProvider<List<PropertyWithListing>>((
  ref,
) async {
  final repo = ref.watch(propertyRepositoryProvider);
...
final recentlyViewedProvider =
    NotifierProvider<RecentlyViewedNotifier, List<PropertyWithListing>>(
