...
app.get('/api/github/repos', async (req, res) => {
  try {
    ...
    const formatted = repos.map(repo => ({
      ...
      stack: Object.entries(repo.language ? { [repo.language]: 1 } : {}).map(([name]) => ({ name, logo: `https://cdn.jsdelivr.net/gh/devicons/devicon/icons/${name.toLowerCase()}/${name.toLowerCase()}-original.svg` })),
