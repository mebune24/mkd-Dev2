  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto relative">
        ...
        >
          <motion.div
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 mb-6 border border-emerald-500/20"
        ...
        >
          <div className="flex flex-col md:flex-row gap-6 items-start">
            ...
            <div className="flex-1">
              <h2 style={{ color: 'var(--text)' }} className="text-2xl md:text-3xl font-bold mb-2">Mebune Donstand</h2>
              <p style={{ color: 'var(--accent)' }} className="text-base md:text-lg font-medium mb-3">Software Engineer · Frontend & Full-Stack</p>
              <p style={{ color: 'var(--muted)' }} className="text-sm md:text-base leading-relaxed mb-4">
                Self-taught software engineer building production-grade web systems with reliability, speed, and strong product focus. Specializing in React, Next.js, Node.js, and modern database architectures.
              </p>
        ...
        <div className="space-y-12 mb-20">
          ...
          >
            ...
            <div className="space-y-4 text-sm md:text-base leading-relaxed" style={{ color: 'var(--muted)' }}>
              <p>
                I'm a <span style={{ color: 'var(--accent)' }} className="font-semibold">self-taught software engineer</span> who builds production-grade web systems for companies that care about reliability and speed. I operate comfortably across product, design, and engineering to ship outcomes that move business metrics.
              </p>
              <p>
                I specialize in modern frontend and full-stack delivery with <span style={{ color: 'var(--accent)' }}>React</span>, <span style={{ color: 'var(--accent)' }}>Next.js</span>, and <span style={{ color: 'var(--accent)' }}>Node.js</span>, with a strong emphasis on system design, accessibility, and performance. My work focuses on scalable architecture, clear component contracts, and maintainable codebases that teams can evolve safely.
              </p>
              <p>
          ...
          {/* Skills & Expertise - Horizontal Journey Style */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
          >
            <h3 style={{ color: 'var(--text)' }} className="text-xl font-bold mb-8 flex items-center gap-2">
              <span style={{ color: 'var(--accent)' }}>#</span> Skills & Expertise
            </h3>
            
            {/* Horizontal Steps with Arrows */}
            <div className="relative">
              {/* Connecting Line */}
              <div className="absolute top-5 left-0 right-0 h-0.5 bg-gradient-to-r from-emerald-500/30 via-cyan-500/30 to-blue-500/30 hidden md:block" />
              
              <div className="flex md:flex-row gap-4 md:gap-0 relative overflow-x-auto md:overflow-visible snap-x snap-mandatory scrollbar-hide -mx-6 px-6 md:mx-0 md:px-0 pb-4 md:pb-0">
                {[
                  { number: "01", title: "Frontend Architecture", description: "React, Next.js, TypeScript", color: "emerald" },
                  { number: "02", title: "Backend Systems", description: "Node.js, Express, GraphQL", color: "cyan" },
                  { number: "03", title: "Database Design", description: "PostgreSQL, MySQL, MongoDB", color: "blue" },
                  { number: "04", title: "DevOps & Tools", description: "Git, Docker, CI/CD", color: "emerald" },
                  { number: "05", title: "Authentication", description: "JWT, OAuth, bcrypt", color: "cyan" },
                  { number: "06", title: "Design Integration", description: "Figma, Tailwind, Bootstrap", color: "blue" }
                ].map((item, index) => (
                  <div key={item.number} className="flex-shrink-0 w-[200px] md:flex-1 relative snap-center">
                    {/* Node */}
                    <motion.div
                    ...
                    >
                      <div className={`w-10 h-10 rounded-full bg-${item.color}-500/20 border-2 border-${item.color}-400 flex items-center justify-center text-${item.color}-400 font-bold text-sm mb-3 shadow-lg shadow-${item.color}-500/20`}>
                        {item.number}
                      </div>
                      <h4 style={{ color: 'var(--text)' }} className="font-semibold text-sm mb-1">{item.title}</h4>
                      <p style={{ color: 'var(--muted)' }} className="text-xs leading-relaxed">{item.description}</p>
                    </motion.div>

                    {/* Arrow between steps */}
                    {index < 5 && (
                      <div className="hidden md:flex absolute top-5 -right-2 transform translate-x-full z-20">
        ...
        >
          ...
          <div className="relative max-w-4xl mx-auto">
            ...
            <svg className="absolute inset-0 w-full h-full hidden md:block" viewBox="0 0 800 200" fill="none" xmlns="http://www.w3.org/2000/svg">
              <ellipse cx="400" cy="100" rx="350" ry="80" stroke="url(#engagementGradient)" strokeWidth="2" strokeDasharray="8 4" opacity="0.5" />
              <defs>
            ...
            {/* Nodes */}
            <div className="flex md:flex-row items-center justify-center gap-6 md:gap-4 relative overflow-x-auto md:overflow-visible snap-x snap-mandatory scrollbar-hide -mx-6 px-6 md:mx-0 md:px-0 pb-4 md:pb-0">
              {[
                ...
                <div key={item.number} className="flex-shrink-0 w-[260px] md:flex-1 relative snap-center">
                  {/* Node */}
                  <motion.div
                    className="flex flex-col items-center text-center relative z-10"
                    initial={{ opacity: 0, scale: 0 }}
